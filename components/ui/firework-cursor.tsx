...
function OriginkitBaseFireworkCursor(props: FireworkCursorProps) {
  ...
  const colors = Array.isArray(colorsProp) && colorsProp.length ? colorsProp : DEFAULT_COLORS;
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      if (over) {
        p.nx = rect.width > 0 ? ((e.clientX - rect.left) / rect.width) * 2 - 1 : 0;
        p.ny = rect.height > 0 ? 1 - ((e.clientY - rect.top) / rect.height) * 2 : 0;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        Particles1Cursor = (mod as any)?.default?.cursors?.particles1 ?? (mod as any)?.cursors?.particles1;
      } catch (e) {
        ...
        particles.update = function patched(this: any, arg: any) {
          ...
          if (pointer) {
            ...
            if (snapRef.current) {
              ...
              const target = particles.uniforms?.uPointerPosition?.value;
              if (worldSize && target) {
                target.set(pointer.nPosition.x * worldSize.wWidth * 0.5, pointer.nPosition.y * worldSize.wHeight * 0.5);
              }
            }
          }
...
export default function FireworkCursor(props: Record<string, unknown>) {
  return <OriginkitBaseFireworkCursor {...(originkitPresetProps as Record<string, unknown>)} {...props} />;
}
