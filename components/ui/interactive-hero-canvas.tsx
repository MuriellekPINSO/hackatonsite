...
function useCanvasAnimation({
  ...
  useEffect(() => {
    ...
    const loop = () => {
      st.frameCount += 1;
      onDrawRef.current(ctx, st) !== false ? (st.animationId = requestAnimationFrame(loop)) : (st.animationId = 0);
    };
...
export default function InteractiveHeroCanvas({
  ...
  useEffect(() => {
    ...
    const onMove = (ev: MouseEvent) => {
      if (stateRef.current.isVisible) mouseRef.current.targetY = ev.clientY - rect.top;
    };
    let rafId = 0;
    const onScroll = () => {
      if (rafId) return;
