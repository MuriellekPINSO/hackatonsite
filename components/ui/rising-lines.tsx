...
/**
 * Rising Lines — a flat horizontal blob of light emitting two layers of rising
 * particles: thin pixel-line "trail" sparks and softer circular glowing blobs.
 * Ported from Originkit (https://www.originkit.dev) — Framer-canvas-only bits
 * (RenderTarget, static-export warm-up) stripped, everything else kept as-is.
 *
...
type Props = {
  className?: string;
  particles?: number; // spark count at the 800×400 reference frame
  color?: string;
...
export default function RisingLines(props: Props) {
  const {
    ...
    showHorizon = DEFAULTS.showHorizon,
    horizonColor = DEFAULTS.horizonColor,
    riseSpeed: riseSpeedRaw = DEFAULTS.riseSpeed,
    opacity: opacityRaw = DEFAULTS.opacity,
    horizonOpacity: horizonOpacityRaw = DEFAULTS.horizonOpacity,
    scale: scaleRaw = DEFAULTS.scale,
    style,
  } = props;
